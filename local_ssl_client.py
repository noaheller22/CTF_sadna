...
def main():
  ...
  parser.add_argument("--public_key_1_path", required=True)
  parser.add_argument("--public_key_2_path", required=True)
  args = parser.parse_args()

  public_key_1 = load_public_key(args.public_key_1_path)
  public_key_2 = load_public_key(args.public_key_2_path)
  ...
  client_key_exchange_1 = build_client_key_exchange(premaster, public_key_1)
  client_key_exchange_2 = build_client_key_exchange(premaster, public_key_2)
  ...
  for j in range(10):
    totals[j] = [0, 0]
    for i in range(250):
      for index, key_exchange in enumerate([client_key_exchange_1, client_key_exchange_2]):
        ...
        finished = build_finished(master_secret, keys, client_hello[5:] + server_hello + key_exchange[5:])
        
        start = time.perf_counter()
        sock.sendall(key_exchange)
        sock.sendall(change_cipher_spec)
        sock.sendall(finished)
        get_server_alert(sock)
        totals[j][index] += time.perf_counter() - start
