...
def main():
    ...
    premaster = TLS_VERSION + urandom(46)  # dummy premaster
    ...
    # Dump raw + parsed response (no classification)
    data = sock.recv(4096)
    print(f"[DEBUG] Raw server response: {data}")
    if len(data) >= 7 and data[0] == 0x15:  # Alert
        version = data[1:3].hex()
        version_major, version_minor = data[1], data[2]
        length = struct.unpack(">H", data[3:5])[0]
        level = data[5]
        levels = {1: "warning", 2: "fatal"}
        description = data[6]
        print(f"[DEBUG] Parsed TLS Alert -> version={version_major}.{version_minor}, length={length}, level={levels.get(level, level)}, description={description}")
    elif data == b"OK\n":
        print("[DEBUG] Server says OK")
