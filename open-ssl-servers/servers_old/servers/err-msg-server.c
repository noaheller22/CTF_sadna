/*  gcc err_oracle.c -o err_oracle -lssl -lcrypto
 *
 *  Listens on TCP 4444.
 *  Protocol: 2-byte big-endian length | ciphertext.
 *  Reply  : if decrypt succeeds → "OK\n"
 *           else → first OpenSSL error string + "\n"
 */
#include <arpa/inet.h>
